<x-moldable::layouts.app>
    ...
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="scroll-reactive-sticky flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex flex-col gap-1.5">
                <div class="flex items-center gap-3">
                    ...
                    <span class="rounded-full bg-slate-100 border border-slate-200/80 px-3.5 py-1 text-xs font-bold text-slate-800 shadow-xs">
                        Moldable CRM v2.0
                    </span>
                </div>
            ...
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 shadow-xs">
                    <label for="entity-selector" class="text-sm font-medium text-slate-500">Target Entity</label>
                    <select id="entity-selector" onchange="MoldableBuilder.setEntity(this.value)" class="rounded-lg border-0 py-0 pl-1 pr-6 text-sm font-bold text-slate-900 bg-transparent outline-none cursor-pointer">
                ...
                <button
                    ...
                    class="rounded-2xl bg-slate-900 hover:bg-black text-white px-6 py-3 text-sm font-bold shadow-md shadow-slate-900/15 transition flex items-center gap-2.5 active:scale-[0.98]"
                >
                    <svg class="w-4.5 h-4.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                    Add New Field
                </button>
            </div>
        </div>
        ...
        <div id="entity-pill-bar" class="flex items-center gap-3.5 overflow-x-auto rounded-2xl border border-slate-200/90 bg-white p-3.5 shadow-xs scrollbar-none">
            <span class="text-xs font-black text-slate-900 px-3 uppercase tracking-wider">Entities</span>
            <div class="flex items-center gap-2.5">
                @foreach(config('moldable.entities', ['leads' => ['name' => 'Leads'], 'persons' => ['name' => 'Persons'], 'organizations' => ['name' => 'Organizations'], 'products' => ['name' => 'Products'], 'quotes' => ['name' => 'Quotes']]) as $code => $meta)
                    <button type="button" onclick="switchEntityPill(this, '{{ $code }}')" data-entity-pill="{{ $code }}" class="entity-pill-btn flex items-center gap-2.5 rounded-xl border {{ $loop->first ? 'border-slate-900 bg-slate-900 text-white shadow-xs' : 'border-slate-200 bg-slate-50/80 text-slate-700 hover:bg-slate-100' }} px-4.5 py-2.5 text-sm font-bold transition">
                        <span>{{ $meta['name'] ?? ucfirst($code) }}</span>
                        <span data-entity-count="{{ $code }}" class="rounded-md {{ $loop->first ? 'bg-white/20 text-white' : 'bg-slate-200/80 text-slate-800' }} px-2 py-0.5 text-xs font-black">0</span>
                    </button>
                @endforeach
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            <div class="lg:col-span-7 xl:col-span-7">
                <div class="h-full rounded-2xl border border-slate-200/90 bg-white p-6 sm:p-7 shadow-xs flex flex-col justify-between space-y-6">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4.5">
                        <div>
                            <h2 class="text-base font-bold text-slate-900">Presentation Groups</h2>
                            <p class="text-sm text-slate-500 mt-0.5">Organize custom fields into structured visual sections</p>
                        </div>
                        <button type="button" onclick="openNewGroupModal()" class="rounded-xl border border-slate-200/90 bg-white px-4 py-2 text-sm font-bold text-slate-800 hover:bg-slate-50 transition flex items-center gap-2 shadow-xs">
                    ...
                    <div id="field-groups-nav" class="space-y-3">
                        <div class="py-8 text-center text-sm text-slate-400">Loading groups…</div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-5 xl:col-span-5 space-y-6">
                ...
                <div class="rounded-2xl border border-slate-200/90 bg-white p-6 sm:p-7 shadow-xs space-y-5">
                    <div>
                        <h2 class="text-base font-bold text-slate-900">Resource Overview</h2>
                        <p class="text-sm text-slate-500 mt-0.5">Resource Metrics &middot; field builder summary</p>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div class="rounded-2xl border border-slate-200/90 bg-gradient-to-br from-white to-slate-50/60 p-4.5 flex items-center gap-4 shadow-xs">
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-slate-900 text-white font-bold shadow-xs">
                            ...
                            <div>
                                <div class="text-3xl font-black text-slate-900" id="stat-total-fields">0</div>
                                <div class="text-sm font-bold text-slate-800">Total Fields</div>
                                <div class="text-xs text-slate-400">For this entity</div>
                            </div>
                        </div>
                        ...
                        <div class="rounded-2xl border border-slate-200/90 bg-gradient-to-br from-white to-slate-50/60 p-4.5 flex items-center gap-4 shadow-xs">
                            ...
                            <div>
                                <div class="text-3xl font-black text-slate-900" id="stat-quick-add">0</div>
                                <div class="text-sm font-bold text-slate-800">Quick Add Fields</div>
                                <div class="text-xs text-slate-400">Faster entry forms</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-2xl border border-slate-200/90 bg-white p-6 sm:p-7 shadow-xs space-y-5">
                    <div>
                        <h2 class="text-base font-bold text-slate-900">Quick Field Types</h2>
                        <p class="text-sm text-slate-500 mt-0.5">Click to add commonly used field types</p>
                    </div>

                    <div class="grid grid-cols-2 gap-3.5">
                        <button type="button" onclick="openAddFieldDrawer('text')" class="flex items-center gap-3.5 rounded-xl border border-slate-200/90 bg-white p-3.5 hover:border-slate-400 hover:shadow-xs transition text-left">
                            <span class="font-black text-xs text-slate-900 bg-slate-100 p-2.5 rounded-xl">Aa</span>
                            <span class="text-sm font-bold text-slate-900">Text</span>
                        </button>
                        <button type="button" onclick="openAddFieldDrawer('select')" class="flex items-center gap-3.5 rounded-xl border border-slate-200/90 bg-white p-3.5 hover:border-slate-400 hover:shadow-xs transition text-left">
                            <span class="font-black text-xs text-slate-900 bg-slate-100 p-2.5 rounded-xl">≡</span>
                            <span class="text-sm font-bold text-slate-900">Select</span>
                        </button>
                        <button type="button" onclick="openAddFieldDrawer('price')" class="flex items-center gap-3.5 rounded-xl border border-slate-200/90 bg-white p-3.5 hover:border-slate-400 hover:shadow-xs transition text-left">
                            <span class="font-black text-xs text-slate-900 bg-slate-100 p-2.5 rounded-xl">$</span>
                            <span class="text-sm font-bold text-slate-900">Price</span>
                        </button>
                        <button type="button" onclick="openAddFieldDrawer('date')" class="flex items-center gap-3.5 rounded-xl border border-slate-200/90 bg-white p-3.5 hover:border-slate-400 hover:shadow-xs transition text-left">
                            <svg class="w-4.5 h-4.5 text-slate-900 bg-slate-100 p-2 rounded-xl box-content" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                            <span class="text-sm font-bold text-slate-900">Date</span>
                        </button>
                        <button type="button" onclick="openAddFieldDrawer('lookup')" class="flex items-center gap-3.5 rounded-xl border border-slate-200/90 bg-white p-3.5 hover:border-slate-400 hover:shadow-xs transition text-left">
                            <svg class="w-4.5 h-4.5 text-slate-900 bg-slate-100 p-2 rounded-xl box-content" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                            <span class="text-sm font-bold text-slate-900">Lookup</span>
                        </button>
                        <button type="button" onclick="openAddFieldDrawer('file')" class="flex items-center gap-3.5 rounded-xl border border-slate-200/90 bg-white p-3.5 hover:border-slate-400 hover:shadow-xs transition text-left">
                            <svg class="w-4.5 h-4.5 text-slate-900 bg-slate-100 p-2 rounded-xl box-content" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                            <span class="text-sm font-bold text-slate-900">File</span>
                        </button>
                    </div>
                </div>
        ...
        <div class="rounded-2xl border border-slate-200/90 bg-white p-6 sm:p-7 shadow-xs space-y-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-5">
                <div class="relative flex-1 max-w-lg">
                    <svg class="w-4.5 h-4.5 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    <input
                        type="text"
                        id="field-search-input"
                        placeholder="Search attributes by title or code..."
                        oninput="filterFields(this.value)"
                        class="w-full rounded-2xl border border-slate-200 bg-slate-50/70 py-3 pl-11 pr-11 text-sm text-slate-900 outline-none transition focus:border-slate-400 focus:bg-white placeholder-slate-400"
                    />
                    <button
                        type="button"
                        id="clear-search-btn"
                        onclick="clearSearch()"
                        class="hidden absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 p-1"
                ...
                <div class="flex items-center gap-2.5">
                    <span class="text-sm font-medium text-slate-500">Active Entity:</span>
                    <span id="selected-entity-badge" class="rounded-xl bg-white px-4 py-2 text-sm font-bold text-slate-900 border border-slate-200/90 shadow-xs">
                        Leads
                    </span>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <h2 id="active-group-header-title" class="text-base font-bold text-slate-900 flex items-center gap-2">
                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                        <span>All Fields ( Leads )</span>
                    </h2>
                    <button type="button" id="reset-group-filter-btn" onclick="MoldableBuilder.clearGroupFilter()" class="hidden rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold px-3 py-1 transition">
                        Show All Fields
                    </button>
                </div>
                <span id="field-count-badge" class="rounded-full bg-slate-100 px-3.5 py-1 text-xs font-bold text-slate-700">0 fields</span>
            </div>

            <!-- Fields Canvas List (Drag & Drop Reorderable) — populated from the API -->
            <div id="fields-list-container" class="space-y-3.5">
                <div class="py-8 text-center text-sm text-slate-400">Loading fields…</div>
            </div>

            <!-- Bottom Add Field Button Center Link -->
            <div class="pt-4 text-center">
                <button type="button" onclick="openAddFieldDrawer()" class="text-sm font-bold text-slate-900 hover:underline inline-flex items-center gap-2">
                    <span class="text-base font-black">+</span> Add New Field
                </button>
            </div>

            <!-- Search Empty State -->
            <div id="no-fields-matched" class="hidden py-14 text-center border border-dashed border-slate-200 rounded-2xl">
                <svg class="w-10 h-10 text-slate-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                <p class="text-sm text-slate-500 font-medium">No matching fields found for your query.</p>
            </div>
            ...
            <div id="no-fields-yet" class="hidden py-14 text-center border border-dashed border-slate-200 rounded-2xl">
                <p class="text-sm text-slate-500 font-medium">No custom fields for this entity yet. Click <span class="font-bold text-slate-900">Add Field</span> to create one.</p>
            </div>
    ...
    <script>
        const MoldableBuilder = (function () {
            ...
            function fieldCard(field) {
                ...
                const badges =
                    (field.is_required ? '<span class="text-xs font-bold text-red-600 bg-red-50/80 border border-red-200/60 px-2.5 py-0.5 rounded-full">• Required</span>' : '') +
                    (field.is_unique ? '<span class="text-xs font-bold text-indigo-600 bg-indigo-50/80 border border-indigo-200/60 px-2.5 py-0.5 rounded-full" data-unique=1>Unique</span>' : '') +
                    (field.quick_add ? '<span class="text-xs font-bold text-slate-700 bg-slate-100 border border-slate-200/60 px-2.5 py-0.5 rounded-full">Quick Add</span>' : '');

                return `
                <div class="field-item flex items-center justify-between rounded-2xl border border-slate-200/90 bg-white p-4.5 transition-all hover:border-slate-400 hover:shadow-xs cursor-grab active:cursor-grabbing" draggable="true" ondragstart="handleDragStart(event)" ondragover="handleDragOver(event)" ondrop="handleDrop(event)" ondragend="handleDragEnd(event)" data-id="${field.id}" data-name="${esc(field.name)}" data-type="${esc(typeLabel)}">
                    <div class="flex items-center gap-4.5">
                        <span class="text-slate-300 select-none text-sm font-bold hover:text-slate-500 cursor-grab">⋮⋮</span>
                        <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-slate-100 text-slate-800 font-black text-sm border border-slate-200/70 shadow-2xs">${esc(icon)}</div>
                        <div>
                            <div class="flex items-center gap-2.5">
                                <h3 class="text-sm font-bold text-slate-900">${esc(field.name)}</h3>
                                ${badges}
                            </div>
                            <p class="text-xs text-slate-400 font-mono mt-0.5">${esc(field.code)}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                        <span class="inline-flex items-center rounded-xl bg-slate-100 px-3.5 py-1.5 text-xs font-bold text-slate-800 border border-slate-200/60">${esc(typeLabel)}</span>
                        <div class="flex items-center gap-2 border-l border-slate-100 pl-3.5">
                            <button type="button" onclick="MoldableBuilder.edit(${field.id})" title="Edit Field" aria-label="Edit Field" class="rounded-xl p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-900 transition">
                                <svg class="w-4.5 h-4.5 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </button>
                            <button type="button" onclick="MoldableBuilder.remove(${field.id})" title="Delete Field" aria-label="Delete Field" class="rounded-xl p-2 text-slate-400 hover:bg-red-50 hover:text-red-600 transition">
                                <svg class="w-4.5 h-4.5 text-slate-600 hover:text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                        </div>
                    </div>
                </div>`;
            ...
            function groupCard(group, count) {
                ...
                return `
                <div class="group-tab-btn flex items-center justify-between rounded-xl border ${isSelected ? 'border-slate-900 ring-2 ring-slate-900/10 bg-slate-50' : 'border-slate-200/90 bg-white hover:border-slate-400'} p-4 transition shadow-xs cursor-pointer" data-group-id="${group.id || ''}">
                    <div class="flex items-center gap-3.5 flex-1" onclick="MoldableBuilder.filterByGroup('${group.id || ''}', '${esc(group.name)}')">
                        <span class="text-slate-300 select-none text-sm font-bold">⋮⋮</span>
                        <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-800 font-bold border border-slate-200/70 shadow-2xs">
                            <svg class="w-4.5 h-4.5 text-slate-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        </div>
                        <div>
                            <h3 class="text-sm font-bold text-slate-900">${esc(group.name)}</h3>
                            <p class="text-xs text-slate-400 mt-0.5">${hasId ? count + ' fields assigned' : 'Suggested group (click to save)'}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2.5">
                        <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">${count}</span>
                        ${hasId ? `
                            <button type="button" onclick="event.stopPropagation(); MoldableBuilder.openAssignModal(${group.id})" title="Assign Fields" aria-label="Assign Fields" class="p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                            </button>
                            <button type="button" onclick="event.stopPropagation(); MoldableBuilder.renameGroup(${group.id}, '${esc(group.name)}')" title="Rename Group" aria-label="Rename Group" class="p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </button>
                            <button type="button" onclick="event.stopPropagation(); MoldableBuilder.deleteGroup(${group.id})" title="Delete Group" aria-label="Delete Group" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                        ` : `
                            <button type="button" onclick="event.stopPropagation(); MoldableBuilder.createGroup('${esc(group.name)}')" class="text-xs font-bold text-slate-900 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-xl transition">
                                Save
                            </button>
                        `}
                    </div>
